    ...
    <body id="bootstrap-override" class="bg-color">
        <?php
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["submit"])) {
                ...
                if (deleteCoupon($id)) {
                    echo '<script>
                    swal("Cupón eliminado!", "Se eliminó el cupón en la base de datos exitosamente.", "success");
                    </script>';
                } else {
                    echo '<script>
                    swal("Lo sentimos...", "Hubo un error al eliminar el cupón.", "error");
                    </script>';
                }
            }
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["search"])){
                ...
                if ($result->num_rows > 0){
                    while ($row = $result->fetch_assoc()) {
                        ...
                        if ($row["general"] == 1) {
                            $general = "Aplicable en cualquier producto";
                        } else {
                            $general = "Aplicable en producto específico";
                        }
                        if ($row["item"] == 0) {
                            $item = "No aplica.";
                        } else {
        ...
        <div class="container mt-3 d-flex flex-column align-items-center px-5">
            <span class="font-paytone text-center color3 h3">Eliminar cupón</span>
                ...
                <form action="<?php echo $_SERVER['PHP_SELF'];?>" method="post" enctype="multipart/form-data" novalidate>
                    <div class="row">
                        ...
                        <div class="col-12 col-md-8 d-flex flex-column justify-content-center">
                            <h5>Nombre del cupón: <?php echo isset($name) ? $name : '' ?></h5>
                        ...
                        <div class="col-12 p-3">
                            <table class="table table-hover table-dark">
                                <tr>
                                    <th>Rango</th>
                                    <td><?php echo isset($general) ? $general : '' ?></td>
                                    <th>Artículo</th>
